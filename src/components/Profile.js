import React from 'react'
import { Grid, Image, Form, Button, Container } from 'semantic-ui-react'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import EditProfileForm from './EditProfileForm'

class Profile extends React.Component {
  constructor() {
    super()
    this.state = {
      edit: false,
      first_name: "",
      last_name: "",
      user_name: "",
      weight: "",
      age: "",
      goal: "",
      gender: "",
      activity_level: "",
      email: "",
      location: "",
      bmr: "",
      calories: "",
      body_fat: ""
    }
  }

  convertToInput = () => {
    let user = this.props.user
    this.setState({
      edit: true,
    })
  }

handleSubmit= () => {
  this.props.logOut()
  this.props.history.push('/login')
}

handleChange = (e) => {
  this.setState({
    [e.target.name]: e.target.value
  })
}

updateUser = (user) => {
  let id = user.id
  fetch(`http://localhost:3001/user/${id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json"
      },
      body: JSON.stringify({
        user: {
          first_name: this.state.first_name,
          last_name: this.state.last_name,
          user_name: this.state.user_name,
          weight: this.state.weight,
          age: this.state.age,
          goal: this.state.goal,
          gender: this.state.gender,
          activity_level: this.state.activity_level,
          email: this.state.email,
          location: this.state.location,
          bmr: this.state.bmr,
          calories: this.state.calories,
          body_fat: this.state.body_fat
        }
      })
    });
}

  render() {

    return (
      <React.Fragment>
        <Container id="profile-image-container">
              <Image id="profile-pic" src='data:image/png;base64,' size='small' circular />
        </Container>

      <Button color="green" onClick={this.convertToInput} user={this.state.user}>Edit Profile</Button>
      <Button color="red" onClick={this.handleSubmit}>Log Out</Button>

      <div id="profile-icons" className="container space-around">
        <FontAwesomeIcon icon="weight" size="3x"/>
        <FontAwesomeIcon icon="plus" size="3x"/>
        <FontAwesomeIcon icon="plus" size="3x"/>
        <FontAwesomeIcon icon="dumbbell" size="3x"/>
      </div>

      {this.state.edit === false ?
      <div>
        {this.props.user ? <h1>Name: {this.props.user.first_name}</h1> : null}
        {this.props.user ? <h1>Last Name: {this.props.user.last_name}</h1> : null}
        {this.props.user ? <h1>Username: {this.props.user.user_name}</h1> : null}
        {this.props.user ? <h1>Email: {this.props.user.email}</h1> : null}
        {this.props.user ? <h1>Age: {this.props.user.age}</h1> : null}
        {this.props.user ? <h1>Weight: {this.props.user.weight}</h1> : null}
        {this.props.user ? <h1>Body Fat: {this.props.user.body_fat}%</h1> : null}
        {this.props.user ? <h1>Location: {this.props.user.location}</h1> : null}
        {this.props.user ? <h1>Goal: {this.props.user.goal}</h1> : null}
        {this.props.user ? <h1>Activity Level: {this.props.user.activity_level}</h1> : null}
        {this.props.user ? <h1>Bmr: {this.props.user.bmr}</h1> : null}
        {this.props.user ? <h1>Calories: {this.props.user.calories}</h1> : null}
        {this.props.user ? <h1>Gender: {this.props.user.gender}</h1> : null}
      </div> : <EditProfileForm user={this.props.user} handleChange={this.handleChange} updateUser={this.updateUser}/>}

      </React.Fragment>

    )
  }
}

export default Profile
